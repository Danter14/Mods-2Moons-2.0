    ...
    function show()
    {
        ...
        foreach ($result as $response) {
            ...
            if (is_null($response['user_quest_users_finish'])) {
                $content_list .= "<span id='quest_badge_" . $response['questsID'] . "' class='badge danger'>{$LNG['que3']}</span>";
            } else if ($response['user_quest_users_finish'] == 1) {
                $content_list .= "<span id='quest_badge_" . $response['questsID'] . "' class='badge warning'>{$LNG['que4']} </span>";
            } else {
                $content_list .= "<span id='quest_badge_" . $response['questsID'] . "' class='badge success'>{$LNG['que5']}</span>";
            }
            $content_list .= "</div>";
            if ($event_finish > 0) {
            ...
            foreach ($list_reward as $key => $reward) {
                if ($reward > 0) {
                    $content_list .= "<li class='reward-quest tooltip' data-tooltip-content='" . number_format($reward) . "'>
                        <div class='ico-quest ico-quest-{$key}'></div>
                        <div class='text-quest'>
                            " . shortly_number($reward) . "
                        </div>
                    <li>";
                }
            }
            ...
            $content_list .= "
                <div class='quest-objectif-content'>
                    <img class='quest-objectif-img' src='./styles/theme/nsc/gebaeude/{$response['quest_objectif']}.gif' alt=''>
                    <div class='quest-objectif-name'>" . $LNG['tech'][$response['quest_objectif']] . "</div>
                    <div class='quest-objectif-number'>+" . number_format($response['quest_objectif_level']) . "</div>
                </div>
            ";
            if (is_null($response['user_quest_users_finish'])) {
                $content_list .= "<div id='quest_button_" . $response['questsID'] . "'><button id='btn_quest_" . $response['questsID'] . "' onclick='javascript:questConfirm(" . $response['questsID'] . ")'>{$LNG['que8']}</button></div>";
            } else {
            ...
            if ($result["quest_actif"] == 1) {
                if ($result['quest_event'] == 1 && $result['quest_time_finish_event'] < TIMESTAMP) {
                    ...
                    if ($result_controle > 1) {
                        ...
                        $response["badgeContent"] = "{$LNG['que4']}";
                    }
                }
            } else {
                $response["error"] = "warning";
