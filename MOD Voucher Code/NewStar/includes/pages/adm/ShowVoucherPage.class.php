...
class ShowVoucherPage extends AbstractAdminPage
{
    ...
    function show()
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($action == "createVoucher") {
                $this->setVoucher($_POST);
            } elseif ($action == "createVoucherReward") {
                $this->setReward($_POST);
            } elseif ($action == "editVoucher") {
                $result = $this->getVoucherById($_POST['id']);
                ...
                $this->display('VoucherEditMods.tpl');
            } elseif ($action == "updateVoucher") {
                $this->updateVoucher($_POST);
            } elseif ($action == "deleteVoucher") {
                $this->deleteVoucher($_POST['id']);
            } elseif ($action == "deleteVoucherUse") {
                $this->deleteUserVoucher($_POST['id']);
            }
        }

        $this->assign(array(
            'vouchers' => $this->getVoucher(false),
            'vouchersList' => $this->getVoucher(true),
            'rewards' => $this->getRewards(),
            'rewardsUser' => $this->getRewardsUser(),
        ));

        $this->display('VoucherMods.tpl');
    }
    ...
    function getVoucher($isComplete = false)
    {
        ...
        $sql = "SELECT * FROM " . VOUCHER . " WHERE start_time <= " . TIMESTAMP . "";
    ...
    function deleteVoucher($id)
    {
        ...
        if (empty($id) || !$this->getVoucherById($id)) {
            return false;
        }
    ...
    function setVoucher($donnes)
    {
        ...
        if ($this->isVerif($donnes)) {
            ...
            $this->saveVoucher($voucher, $start_time, $end_time);
        }
    }
    ...
    function setReward($donnes)
    {
        ...
        if ($this->isVerifReward($donnes)) {
            $this->saveReward($voucher_code, $reward, $quantity);
        }
    }
    ...
    {
        if (empty($id) || !$this->getVoucherUseById($id) || !is_numeric($id)) {
            return false;
        }
